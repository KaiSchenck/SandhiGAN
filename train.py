...
from model_tf import Generator, Discriminator, Generator_f0, Discriminator_f0
...
class CycleGANTraining(object):
    def __init__(self,
        ...
        self.f0_A = self.loadPickleFile(f0s_A)
        self.f0_B = self.loadPickleFile(f0s_B)
        
        self.dataset_A = list(zip(self.dataset_A, self.f0_A))
        self.dataset_B = list(zip(self.dataset_B, self.f0_B))
        ...
        self.generator_A2B_f0 = Generator_f0().to(self.device)
        self.generator_B2A_f0 = Generator_f0().to(self.device)
        self.discriminator_A_f0 = Discriminator_f0().to(self.device)
        self.discriminator_B_f0 = Discriminator_f0().to(self.device)
        ...
        g_params_f0 = list(self.generator_A2B_f0.parameters()) + \
                   list(self.generator_B2A_f0.parameters())
        d_params_f0 = list(self.discriminator_A_f0.parameters()) + \
                   list(self.discriminator_B_f0.parameters())
        ...
        self.generator_f0_optimizer = torch.optim.Adam(
            g_params_f0, lr=self.generator_lr, betas=(0.5, 0.999))
        self.discriminator_f0_optimizer = torch.optim.Adam(
            d_params_f0, lr=self.discriminator_lr, betas=(0.5, 0.999))
        ...
        self.generator_f0_loss_store = []
        self.discriminator_f0_loss_store = []
        ...
        if restart_training_at is not None:
            ...
            for i in range(10):
                self.start_epoch = self.loadModel('./l_checkpoint/_CycleGAN_CheckPoint_60')
                self.output_A_dir = "./epoch_val/e60_trial"
                self.validation_for_A_dir(i)
    ...
    def reset_grad(self):
        ...
        self.generator_f0_optimizer.zero_grad()
        self.discriminator_f0_optimizer.zero_grad()

    def train(self):
        # Training Begins
        #self.validation_for_A_dir(0)
        ...
        for epoch in range(self.start_epoch, self.num_epochs):
            ...
            for i, (real_A, real_B) in enumerate(train_loader):
                ...
                real_A_f0 = real_A[1].to(self.device).float()
                real_B_f0 = real_B[1].to(self.device).float()
                
                real_A = real_A[0].to(self.device).float()
                real_B = real_B[0].to(self.device).float()
                
                #print("Load")
                # Generator Loss function

                fake_B = self.generator_A2B(real_A)
                fake_B_f0 = self.generator_A2B_f0(real_A_f0)
                
                cycle_A = self.generator_B2A(fake_B)
                cycle_A_f0 = self.generator_B2A_f0(fake_B_f0)
                
                fake_A = self.generator_B2A(real_B)
                fake_A_f0 = self.generator_B2A_f0(real_B_f0)
                
                cycle_B = self.generator_A2B(fake_A)
                cycle_B_f0 = self.generator_A2B_f0(fake_A_f0)

                identity_A = self.generator_B2A(real_A)
                identity_A_f0 = self.generator_B2A_f0(real_A_f0)
                
                identity_B = self.generator_A2B(real_B)
                identity_B_f0 = self.generator_A2B_f0(real_B_f0)
                
                #print("Gen")

                d_fake_A = self.discriminator_A(fake_A)
                d_fake_A_f0 = self.discriminator_A_f0(fake_A_f0)
                
                d_fake_B = self.discriminator_B(fake_B)
                d_fake_B_f0 = self.discriminator_B(fake_B_f0)

                # for the second step adverserial loss
                d_fake_cycle_A = self.discriminator_A(cycle_A)
                d_fake_cycle_A_f0 = self.discriminator_A_f0(cycle_A_f0)
                
                d_fake_cycle_B = self.discriminator_B(cycle_B)
                d_fake_cycle_B_f0 = self.discriminator_B_f0(cycle_B_f0)
                ...
                cycleLoss_f0 = torch.mean(
                    torch.abs(real_A_f0 - cycle_A_f0)) + torch.mean(torch.abs(real_B_f0 - cycle_B_f0))
                ...
                identiyLoss_f0 = torch.mean(
                    torch.abs(real_A_f0 - identity_A_f0)) + torch.mean(torch.abs(real_B_f0 - identity_B_f0))

                # Generator Loss
                generator_loss_A2B = torch.mean((1 - d_fake_B) ** 2)
                generator_loss_B2A = torch.mean((1 - d_fake_A) ** 2)
                
                generator_loss_A2B_f0 = torch.mean((1 - d_fake_B_f0) ** 2)
                generator_loss_B2A_f0 = torch.mean((1 - d_fake_A_f0) ** 2)
                ...
                generator_loss_f0 = generator_loss_A2B_f0 + generator_loss_B2A_f0 + \
                                 cycle_loss_lambda * cycleLoss_f0 + identity_loss_lambda * identiyLoss_f0
                self.generator_f0_loss_store.append(generator_loss_f0.item())
            
                # Backprop for Generator
                self.reset_grad()
                generator_loss.backward()
                generator_loss_f0.backward()
                ...
                self.generator_f0_optimizer.step()
                
                # Discriminator Loss Function

                # Discriminator Feed Forward
                d_real_A = self.discriminator_A(real_A)
                d_real_A_f0 = self.discriminator_A_f0(real_A_f0)
                
                d_real_B = self.discriminator_B(real_B)
                d_real_B_f0 = self.discriminator_B_f0(real_B_f0)

                generated_A = self.generator_B2A(real_B)
                generated_A_f0 = self.generator_B2A_f0(real_B_f0)
                d_fake_A = self.discriminator_A(generated_A)
                d_fake_A_f0 = self.discriminator_A_f0(generated_A_f0)

                # for the second step adverserial loss
                cycled_B = self.generator_A2B(generated_A)
                cycled_B_f0 = self.generator_A2B_f0(generated_A_f0)
                
                d_cycled_B = self.discriminator_B(cycled_B)
                d_cycled_B_f0 = self.discriminator_B_f0(cycled_B_f0)
                
                generated_B = self.generator_A2B(real_A)
                generated_B_f0 = self.generator_A2B_f0(real_A_f0)
                
                d_fake_B = self.discriminator_B(generated_B)
                d_fake_B_f0 = self.discriminator_B_f0(generated_B_f0)

                #print("Feed forward")

                # for the second step adverserial loss
                cycled_A = self.generator_B2A(generated_B)
                cycled_A_f0 = self.generator_B2A_f0(generated_B_f0)
                
                d_cycled_A = self.discriminator_A(cycled_A)
                d_cycled_A_f0 = self.discriminator_A_f0(cycled_A_f0)
                ...
                d_loss_A_real_f0 = torch.mean((1 - d_real_A_f0) ** 2)
                d_loss_A_fake_f0 = torch.mean((0 - d_fake_A_f0) ** 2)
                d_loss_A_f0 = (d_loss_A_real_f0 + d_loss_A_fake_f0) / 2.0

                d_loss_B_real = torch.mean((1 - d_real_B) ** 2)
                d_loss_B_fake = torch.mean((0 - d_fake_B) ** 2)
                d_loss_B = (d_loss_B_real + d_loss_B_fake) / 2.0
                
                d_loss_B_real_f0 = torch.mean((1 - d_real_B_f0) ** 2)
                d_loss_B_fake_f0 = torch.mean((0 - d_fake_B_f0) ** 2)
                d_loss_B_f0 = (d_loss_B_real_f0 + d_loss_B_fake_f0) / 2.0
                ...
                d_loss_A_cycled_f0 = torch.mean((0 - d_cycled_A_f0) ** 2)
                d_loss_B_cycled_f0 = torch.mean((0 - d_cycled_B_f0) ** 2)
                d_loss_A_2nd_f0 = (d_loss_A_real + d_loss_A_cycled_f0) / 2.0
                d_loss_B_2nd_f0 = (d_loss_B_real + d_loss_B_cycled_f0) / 2.0
                ...
                d_loss_f0 = (d_loss_A_f0 + d_loss_B_f0) / 2.0 + (d_loss_A_2nd_f0 + d_loss_B_2nd_f0) / 2.0
                self.discriminator_f0_loss_store.append(d_loss_f0.item())

                #print("Adverserial loss")
                ...
                self.discriminator_f0_optimizer.step()

                if i % 5 == 0 and i != 0:
                    pbar.set_description(
                        "Epoch:{} Iter:{} G Loss:{:.4f} D Loss:{:.4f} G_f0 Loss:{:.4f} D_f0 Loss:{:.4f} GA2B:{:.4f} GB2A:{:.4f} G_id:{:.4f} G_cyc:{:.4f} D_A:{:.4f} D_B:{:.4f}".format(
                            epoch,
                            num_iterations,
                            generator_loss.item(),
                            # loss['generator_loss'],
                            d_loss.item(),
                            generator_loss_f0.item(),
                            d_loss_f0.item(),
                            generator_loss_A2B_f0, generator_loss_B2A_f0, identiyLoss, cycleLoss, d_loss_A,
                            d_loss_B))
            ...
            if epoch % 5 == 0 and epoch != 0:
                end_time = time.time()
                store_to_file = "Epoch: {} G Loss: {:.4f} D Loss: {}, G_f0 Loss: {:.4f} D_f0 Loss: {}, Time: {:.2f}\n\n".format(
                    epoch, generator_loss.item(), d_loss.item(), generator_loss_f0.item(), d_loss_f0.item(), end_time - start_time_epoch)
                self.store_to_file(store_to_file)
                print("Epoch: {} G Loss: {:.4f} D Loss: {}, G_f0 Loss: {:.4f} D_f0 Loss: {}, Time: {:.2f}\n\n".format(
                    epoch, generator_loss.item(), d_loss.item(), generator_loss_f0.item(), d_loss_f0.item(), end_time - start_time_epoch))
    ...
    def validation_for_A_dir(self, epoch):
        ...
        for i, file in enumerate(preprocess.load_wavs(validation_A_dir)):
            #filePath = os.path.join(validation_A_dir, file)
            #wav, _ = librosa.load(filePath, sr=sampling_rate, mono=True)
            wav = preprocess.wav_padding(wav=file,
                                         sr=sampling_rate,
            ...
            f0_converted = torch.from_numpy(f0_converted).float()
            f0_converted = f0_converted.unsqueeze(0)
            
            coded_sp_converted_norm = self.generator_A2B(coded_sp_norm)
            f0_norm = self.generator_A2B_f0(f0_converted)
            
            coded_sp_converted_norm, f0_norm = coded_sp_converted_norm.cpu().detach().numpy(), f0_norm.cpu().detach().numpy()
            coded_sp_converted_norm, f0_norm = np.squeeze(coded_sp_converted_norm), np.squeeze(f0_norm)
            
            f0_norm = f0_norm.astype(np.double)
            ...
            wav_transformed = preprocess.world_speech_synthesis(f0=f0_norm,
                                                                decoded_sp=decoded_sp_converted,
                                                                ap=ap,
                                                                fs=sampling_rate,
                                                                frame_period=frame_period)
            sf.write(file=os.path.join(output_A_dir, "e" + str(epoch) + "_output_" + str(i) + ".wav"),
    ...
    def saveModelCheckPoint(self, epoch, PATH):
        torch.save({
            ...
            'generator_f0_loss_store': self.generator_f0_loss_store,
            'discriminator_f0_loss_store': self.discriminator_f0_loss_store,
            'discriminator_loss_store': self.discriminator_loss_store,
            ...
            'model_genA2B_f0_state_dict': self.generator_A2B_f0.state_dict(),
            'model_genB2A_f0_state_dict': self.generator_B2A_f0.state_dict(),
            'model_discriminatorA_f0': self.discriminator_A_f0.state_dict(),
            'model_discriminatorB_f0': self.discriminator_B_f0.state_dict(),
            'generator_optimizer': self.generator_optimizer.state_dict(),
            'discriminator_optimizer': self.discriminator_optimizer.state_dict(),
            'generator_f0_optimizer': self.generator_f0_optimizer.state_dict(),
            'discriminator_f0_optimizer': self.discriminator_f0_optimizer.state_dict()
        }, PATH)
    ...
    def loadModel(self, PATH):
        ...
        self.generator_A2B_f0.load_state_dict(
            state_dict=checkPoint['model_genA2B_f0_state_dict'])
        self.generator_B2A_f0.load_state_dict(
            state_dict=checkPoint['model_genB2A_f0_state_dict'])
        self.discriminator_A_f0.load_state_dict(
            state_dict=checkPoint['model_discriminatorA_f0'])
        self.discriminator_B_f0.load_state_dict(
            state_dict=checkPoint['model_discriminatorB_f0'])
        self.generator_optimizer.load_state_dict(
            state_dict=checkPoint['generator_optimizer'])
        self.discriminator_optimizer.load_state_dict(
            state_dict=checkPoint['discriminator_optimizer'])
        self.generator_f0_optimizer.load_state_dict(
            state_dict=checkPoint['generator_f0_optimizer'])
        self.discriminator_f0_optimizer.load_state_dict(
            state_dict=checkPoint['discriminator_f0_optimizer'])
        ...
        self.generator_f0_loss_store = checkPoint['generator_loss_store']
        self.discriminator_f0_loss_store = checkPoint['discriminator_loss_store']
        return epoch
...
if __name__ == '__main__':
    ...
    logf0s_normalization_default = '/logf0s_normalization.npz'
    mcep_normalization_default = '/mcep_normalization.npz'
    coded_sps_A_norm = '/coded_sps_A_norm.pickle'
    coded_sps_B_norm = '/coded_sps_B_norm.pickle'
    
    cache_default = './cache'
    
    f0s_A = '/f0s_A.pickle'
    f0s_B = '/f0s_B.pickle'
    model_checkpoint = 'l_checkpoint/'
    resume_training_at = './l_checkpoint/_CycleGAN_CheckPoint_30'
    #resume_training_at = None

    validation_A_dir_default = './a_wavs_val_1.npy'
    output_A_dir_default = './epoch_val/e30_norm/'

    validation_B_dir_default = './b_wavs_val.npy'
    output_B_dir_default = './converted_sound/B/'
    
    parser.add_argument('--cache', type=str,
                        help="File directory of the cache", default=cache_default)
    ...
    logf0s_normalization = argv.cache + logf0s_normalization_default
    mcep_normalization = argv.cache + mcep_normalization_default
    coded_sps_A_norm = argv.cache + coded_sps_A_norm
    coded_sps_B_norm = argv.cache + coded_sps_B_norm
    f0s_A = argv.cache + f0s_A
    f0s_B = argv.cache + f0s_B
